      ...
      {posts.map((post) => (
        <Link href={{pathname:`/post/${post?.slug?.current}` , query:{ slug:post?.slug?.current},
          ...
          <div className="flex flex-col md:flex-row
           gap-10 bg-white rounded-md rounded-tr-md rounded-br-md hover:shadow-md duration-200">

            <div className="w-full md:w-3/5 group overflow-hidden rounded-tl-md rounded-bl-md relative">

              <Image src={urlFor(post?.mainImage).url()} width={300} height={500} alt="blog post image" className="w-full max-h-[500px] object-cover group-hover:scale-105 duration-500 rounded-tl-md rounded-bl-md" />

              <div className="absolute top-0 left-0 bg-black/25 w-full h-full group-hover:hidden duration-200" />
            ...
            {post?.categories?.map((item) => (
              <p key={item?._id}
              className="text-xs uppercase text-blue-600 font-bold">{item?.title}</p>
            ))}
              </div>

                <h2 className="text-2xl font-semibold hover:text-orange-800 duration-200 marker:cursor-pointer">{post?.title}</h2>

                <p className="text-gray-5">{post?.description}</p>
